...
const ScoreScreen = () => {
  ...
  useEffect(() => {
    dispatch(setQuizHeader("Truthy Falsy Quiz Score"))
        ...
        {questions.map((q) => {
          ...
          return (
            <ListGroupItem>
              ...
              <FontAwesomeIcon icon={correct ? faCheckCircle : faTimesCircle} color={correct ? "green" : "red"} size="lg" className="mr-2"/>
              {parse(q.question)}
              </ListGroupItemHeading>
              <ListGroupItemText>
                <p>Correct answer: <span className="font-weight-bold">{q.correct_answer}</span></p>
              </ListGroupItemText>
            </ListGroupItem>
          )
        })}
